...
/** Contains freshly joined person to voice channel defined by the channelId (second parameter) */
const freshJoin: Map<string, string> = new Map();
...
export function recordController(connection: VoiceConnection, member: GuildMember, userId: string, recordableRole: string) {
    const canRecord = !connection.receiver.subscriptions.has(userId) && // Not already recording
        (recordableRole === '' || member.roles.cache.some(role => role.id === recordableRole)) && // Can record this member
        !member.user.bot &&
        (Math.random() < PROBABILITY_TO_RECORD || freshJoin.get(userId) === connection.joinConfig.channelId); // Freshly joined or got lucky to record
...
export function markNewJoin(channelId: string, userId: string) {
    freshJoin.set(userId, channelId);
    // Mark not new anymore after some time
    setTimeout(() => freshJoin.delete(userId), FRESH_PERSON_COOLDOWN_MS);
}
